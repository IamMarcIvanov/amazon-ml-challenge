...
datapath = r'/mnt/d/amazon-ml/dataset/Processed/train2.csv'

data = pd.read_csv(datapath, nrows=100000)
train, test = train_test_split(data, test_size=0.2)
node_grp = train.groupby('BROWSE_NODE_ID', axis='index')
column_used = 'TITLE'
x = node_grp.apply(lambda series: [series[0], series[1]])
known_brands = node_grp.apply(lambda row: {val: key for key, val in row}).to_dict()
print(known_brands)
#using brand as fool proof method to identify if there exists a product in a category


def score_individual_group_on_desc(row_words, group_words):
    """
...
def test_predictions(test_data, num_of_rows):
    ...
    for i in range(num_of_rows):
        ...
        if test_data.iloc[i]['BRAND'] in known_brands:
            predicted_group_label = known_brands[test_data.iloc[i]['BRAND']]
